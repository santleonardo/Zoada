...
const UploadMusicPanel: React.FC<UploadMusicPanelProps> = ({ userName, onProfileChange }) => {
  ...
  // Músicas já enviadas anteriormente por esse artista (pra poder apagar).
  ...
  const fetchMyTracks = async (id: string) => {
    setLoadingMyTracks(true);
    try {
      const res = await fetch(`/api/tracks?artist_id=${encodeURIComponent(id)}`);
      const data = await res.json();
      if (Array.isArray(data.tracks)) setMyTracks(data.tracks);
    } catch {
      // Não é crítico pra tela de upload — só deixa a lista vazia.
    } finally {
  ...
  const handleDeleteTrack = async (trackId: string) => {
    ...
    try {
      ...
      setDeleteError(err instanceof Error ? err.message : 'Erro ao apagar faixa');
    } finally {
      setDeletingId(null);
  ...
  return (
    <div className="rounded-2xl bg-[#1E2030] p-5 mb-6">
      ...
      {/* Músicas já enviadas — com opção de apagar */}
      {(loadingMyTracks || myTracks.length > 0) && (
        <div className="rounded-xl bg-white/5 p-4 mb-4">
          <p className="text-sm font-semibold text-white/80 mb-3">Suas músicas enviadas</p>

          {deleteError && (
            <p className="text-xs text-[#E84393] mb-2">{deleteError}</p>
          )}
          ...
          {loadingMyTracks && myTracks.length === 0 ? (
            ...
            <div className="space-y-2">
              {myTracks.map((track) => (
                <div key={track.id} className="flex items-center gap-3 px-3 py-2 rounded-xl bg-white/5">
                  <div className="w-10 h-10 rounded-lg bg-[#252840] overflow-hidden flex-shrink-0 flex items-center justify-center">
                    {track.cover_url ? (
                      // eslint-disable-next-line @next/next/no-img-element
                  ...
                  {confirmDeleteId === track.id ? (
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      {deletingId === track.id ? (
                        ...
                        <>
                          <span className="text-[11px] text-white/40 mr-1">Apagar?</span>
                          <button
