...
interface ClubPostCommentThreadProps {
  clubPostId: string;
  /** Contagem vinda da própria postagem (evita começar em "0 comentários" piscando enquanto carrega). */
  initialCount?: number;
}

/**
 * Thread de comentários de uma postagem do mural de um clube — mesmo
 * mecanismo de PostCommentThread (feed geral), só que restrita aos
 * membros do clube (a API já confere isso em /api/club-post-comments).
 * Botão com contador que expande a lista (em ordem cronológica) e um
 * campo pra comentar, embaixo de cada postagem no mural.
 */
const ClubPostCommentThread: React.FC<ClubPostCommentThreadProps> = ({ clubPostId, initialCount = 0 }) => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    fetchClubPostComments(clubPostId).then((data) => {
      if (cancelled) return;
      setComments(data);
  ...
  const handleSend = async () => {
    ...
    setNewComment('');
    setIsSending(true);
    const result = await postClubPostComment(clubPostId, content);
    setIsSending(false);
  ...
  const handleReact = async (commentId: string) => {
    ...
    const result = await toggleClubPostCommentLike(commentId);
  ...
  const handleDeleteComment = async (commentId: string) => {
    ...
    const ok = await deleteClubPostComment(commentId);
    setDeletingId(null);
  ...
  return (
    <div>
      ...
      <ReportModal
        ...
        targetType="COMENTARIO_POSTAGEM_CLUBE"
...
export default ClubPostCommentThread;
