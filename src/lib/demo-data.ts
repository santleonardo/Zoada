import type { Track, Artist, User, Comment, Conversation, Message } from '@/types';

// ============================================================
// DADOS DE DEMONSTRAÇÃO
// Usados quando o Supabase não está configurado.
// ============================================================

export const DEMO_ARTISTS: Artist[] = [
  {
    id: 'artist-1',
    name: 'DJ Thunder',
    avatar_url: '/demo-artist-1.jpg',
    cover_url: '/demo-cover-1.jpg',
    bio: 'Eletrônica | Bass Music',
    genre: 'Eletrônica',
    followers_count: 12400,
  },
  {
    id: 'artist-2',
    name: 'Luna Vox',
    avatar_url: '/demo-artist-2.jpg',
    cover_url: '/demo-cover-2.jpg',
    bio: 'Indie | Dream Pop',
    genre: 'Indie',
    followers_count: 8900,
  },
  {
    id: 'artist-3',
    name: 'MC Flow',
    avatar_url: '/demo-artist-3.jpg',
    cover_url: '/demo-cover-3.jpg',
    bio: 'Hip Hop | Trap',
    genre: 'Hip Hop',
    followers_count: 23100,
  },
  {
    id: 'artist-4',
    name: 'Neon Pulse',
    avatar_url: '/demo-artist-4.jpg',
    cover_url: '/demo-cover-4.jpg',
    bio: 'Synthwave | Retro',
    genre: 'Synthwave',
    followers_count: 5600,
  },
  {
    id: 'artist-5',
    name: 'Sombra',
    avatar_url: '/demo-artist-5.jpg',
    cover_url: '/demo-cover-5.jpg',
    bio: 'R&B | Soul',
    genre: 'R&B',
    followers_count: 15200,
  },
  {
    id: 'artist-6',
    name: 'Banda Aurora',
    avatar_url: '/demo-artist-6.jpg',
    cover_url: '/demo-cover-6.jpg',
    bio: 'Rock Alternativo',
    genre: 'Rock',
    followers_count: 9800,
  },
  {
    id: 'artist-7',
    name: 'Kiko Beatz',
    avatar_url: '/demo-artist-7.jpg',
    cover_url: '/demo-cover-7.jpg',
    bio: 'Funk | Bass',
    genre: 'Funk',
    followers_count: 31200,
  },
  {
    id: 'artist-8',
    name: 'Velvet Skies',
    avatar_url: '/demo-artist-8.jpg',
    cover_url: '/demo-cover-8.jpg',
    bio: 'Lo-Fi | Chill',
    genre: 'Lo-Fi',
    followers_count: 18500,
  },
];

// As faixas de demonstração (placeholders com áudio do SoundHelix) foram
// removidas. Agora o app usa apenas faixas reais vindas do banco de dados.
export const DEMO_TRACKS: Track[] = [];

export const DEMO_COMMENTS: Comment[] = [
  {
    id: 'comment-1',
    user_id: 'user-2',
    track_id: 'track-1',
    content: 'Essa batida é insana! 🔥',
    created_at: '2025-06-20T14:30:00Z',
    user: {
      id: 'user-2',
      email: 'maria@email.com',
      name: 'Maria Silva',
      avatar_url: null,
      created_at: '2025-01-01T00:00:00Z',
    },
  },
  {
    id: 'comment-2',
    user_id: 'user-3',
    track_id: 'track-1',
    content: 'Tô ouvindo em loop já',
    created_at: '2025-06-20T15:45:00Z',
    user: {
      id: 'user-3',
      email: 'pedro@email.com',
      name: 'Pedro Santos',
      avatar_url: null,
      created_at: '2025-01-05T00:00:00Z',
    },
  },
  {
    id: 'comment-3',
    user_id: 'user-2',
    track_id: 'track-3',
    content: 'MC Flow sempre entregando! 💪',
    created_at: '2025-06-21T10:00:00Z',
    user: {
      id: 'user-2',
      email: 'maria@email.com',
      name: 'Maria Silva',
      avatar_url: null,
      created_at: '2025-01-01T00:00:00Z',
    },
  },
];

export const DEMO_USER: User = {
  id: 'user-1',
  email: 'demo@zoada.com',
  name: 'Demo User',
  avatar_url: null,
  created_at: '2025-01-01T00:00:00Z',
};

export const DEMO_CONVERSATIONS: Conversation[] = [
  {
    id: 'conv-1',
    user_id: 'user-1',
    other_user: {
      id: 'user-2',
      email: 'maria@email.com',
      name: 'Maria Silva',
      avatar_url: null,
      created_at: '2025-01-01T00:00:00Z',
    },
    last_message: {
      id: 'msg-1',
      sender_id: 'user-2',
      receiver_id: 'user-1',
      content: 'Oiii! Ouviu o novo do DJ Thunder?',
      read: true,
      created_at: '2025-06-21T16:00:00Z',
    },
    unread_count: 0,
  },
  {
    id: 'conv-2',
    user_id: 'user-1',
    other_user: {
      id: 'user-3',
      email: 'pedro@email.com',
      name: 'Pedro Santos',
      avatar_url: null,
      created_at: '2025-01-05T00:00:00Z',
    },
    last_message: {
      id: 'msg-2',
      sender_id: 'user-3',
      receiver_id: 'user-1',
      content: 'Bora pro show amanhã?',
      read: false,
      created_at: '2025-06-22T09:30:00Z',
    },
    unread_count: 1,
  },
  {
    id: 'conv-3',
    user_id: 'user-1',
    other_user: {
      id: 'user-4',
      email: 'ana@email.com',
      name: 'Ana Costa',
      avatar_url: null,
      created_at: '2025-02-10T00:00:00Z',
    },
    last_message: {
      id: 'msg-3',
      sender_id: 'user-1',
      receiver_id: 'user-4',
      content: 'Sim, essa playlist tá perfeita!',
      read: true,
      created_at: '2025-06-21T20:00:00Z',
    },
    unread_count: 0,
  },
];

export const DEMO_MESSAGES: Record<string, Message[]> = {
  'conv-1': [
    {
      id: 'msg-1',
      sender_id: 'user-2',
      receiver_id: 'user-1',
      content: 'Oiii! Ouviu o novo do DJ Thunder?',
      read: true,
      created_at: '2025-06-21T16:00:00Z',
    },
    {
      id: 'msg-1b',
      sender_id: 'user-1',
      receiver_id: 'user-2',
      content: 'Oiii! Sim, Bass Drop tá muito boa! 🔥',
      read: true,
      created_at: '2025-06-21T16:05:00Z',
    },
    {
      id: 'msg-1c',
      sender_id: 'user-2',
      receiver_id: 'user-1',
      content: 'Eu sei!! Tô ouvindo em loop haha',
      read: true,
      created_at: '2025-06-21T16:10:00Z',
    },
    {
      id: 'msg-1d',
      sender_id: 'user-1',
      receiver_id: 'user-2',
      content: 'A gente devia montar uma playlist juntos',
      read: true,
      created_at: '2025-06-21T16:15:00Z',
    },
  ],
  'conv-2': [
    {
      id: 'msg-2',
      sender_id: 'user-3',
      receiver_id: 'user-1',
      content: 'Bora pro show amanhã?',
      read: false,
      created_at: '2025-06-22T09:30:00Z',
    },
  ],
  'conv-3': [
    {
      id: 'msg-3a',
      sender_id: 'user-4',
      receiver_id: 'user-1',
      content: 'Oi! Vi que você curtiu a playlist do Neon Pulse',
      read: true,
      created_at: '2025-06-21T19:50:00Z',
    },
    {
      id: 'msg-3',
      sender_id: 'user-1',
      receiver_id: 'user-4',
      content: 'Sim, essa playlist tá perfeita!',
      read: true,
      created_at: '2025-06-21T20:00:00Z',
    },
  ],
};

// Color palette for generated cover art
export const COVER_COLORS = [
  ['#FF8C42', '#E84393'],
  ['#6C5CE7', '#00CEC9'],
  ['#E84393', '#6C5CE7'],
  ['#FDCB6E', '#FF8C42'],
  ['#00CEC9', '#6C5CE7'],
  ['#E84393', '#FF8C42'],
  ['#6C5CE7', '#E84393'],
  ['#FDCB6E', '#6C5CE7'],
];
